...
export default function Admin(){
  ...
  useEffect(() => {
    // GitHub Pages: no backend. Read from localStorage only.
    try {
      const raw = localStorage.getItem('dpdp_dsars');
      const list = raw ? JSON.parse(raw) : [];
      setDsars(Array.isArray(list) ? list : []);
    } catch {
      setDsars([]);
    }
    // Merge any locally saved consent with an empty server list
    setConsents(mergeWithLocalConsent([]));
  }, []);
