...
export default function DsarSubmit() {
  ...
  async function submit(e) {
    e.preventDefault();
    // GitHub Pages compatible: store locally (no backend)
    const id = 'DSAR-' + Math.floor(Date.now() / 1000);
    const record = {
      id,
    ...
    try {
      const raw = localStorage.getItem('dpdp_dsars');
      const list = raw ? JSON.parse(raw) : [];
      list.unshift(record);
      localStorage.setItem('dpdp_dsars', JSON.stringify(list));
      alert('DSAR submitted: ' + id);
      setForm({ principal_identifier: '', contactEmail: '', type: 'access', details: '' });
    } catch (err) {
      console.error('Failed to save DSAR locally', err);
